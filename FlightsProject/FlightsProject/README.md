...
To update the database use this command from the Web project folder (replace `FlightsProject` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../FlightsProject.Infrastructure/FlightsProject.Infrastructure.csproj -s FlightsProject.Web.csproj
```
